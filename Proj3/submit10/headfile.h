...
//EFFECT:	SEARCH THE RESULT FOR CATEGORY BETWEEN CATEGORY1 AND CATEGORY2 AND SAVE IT TO THE RECENT VECTOR
//REQUIRED:	THE SORTED VECTOR<ENTRY*> TYPE DATA, TWO BOUNDARY CATEGORY1 AND CATEGORY2 AND THE VECTOR<ENTRY*> RECENT
//MODIFY: 	NONE

void parse(entry* tempdata,vector<keywordarray*> hashtable[],int hashsize);
//EFFECT:	GET AN ENTRY AND PARSE THE KEYWORD, SAVING THEM TO THE HASHTABLE
//REQUIRED:	THE VECTOR<KEYWORDARRAY*> HASHTABLE, THE INT HASHSIZE AND THE ENTRY POINTER
//MODIFY:	THE HASHTABLE

void put_in(const string &keyword,vector<keywordarray*> hashtable[],int hashsize,entry* data);
//EFFECT:	PUT THE STRING INTO THE HASHTABLE
//REQUIRED:	THE STRING KEYWORD, THE HASHTABLE AND THE ENTRY* DATA
//MODIFY:	THE HASHTABLE

bool getTpair(pair<string, string> &temp, const string &input);
//EFFECT:	PARSE THE INPUT THE TIMESTAMP PAIR, RETURN 1 IF IT IS VALID, RETURN 0 IF IT IS NOT VALID
//REQUIRED:	THE STRING INPUT, AND THE PAIR TEMP
//MODIFY:	THE PAIR<STRING,STRING> TEMP

bool getCpair(pair<string, string> &temp, const string &input);
//EFFECT:	PARSE THE INPUT THE CATEGORY PAIR, RETURN 1 IF IT IS VALID, RETURN 0 IF IT IS NOT VALID
//REQUIRED:	THE STRING INPUT, AND THE PAIR TEMP
//MODIFY:	THE PAIR<STRING,STRING> TEMP

void parsekeyword(string &keysentence, vector<string> &result);
//EFFECT:	PARSE THE KEYWORD INTO THE RESULT VECTOR
//REQUIRED:	THE KEYSENTENCE AND VECTOR<STRING> RESULT
//MODIFY:	THE VECTOR<STRING> RESULT

vector<entry*> stl_intersection(vector<entry*> &data1, vector<entry*> &data2);
//EFFECT:	DO INTERSECTION OF THE DATA1 AND DATA2, RETURN RESULT
//REQUIRED: 	TWO VECTOR<ENTRY*> TYPE DATA TYPE
//MODIFY:	NONE
