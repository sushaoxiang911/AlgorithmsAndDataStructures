...
//	cout<<getTpair(temp, "::::|01:23:12:43:32")<<endl;
...
//				}
				...
				for(int i=0;i<recent.size();i++)
				{
					cout<<recent[i]->id<<"|"<<recent[i]->timestamp<<"|"<<recent[i]->category<<"|"<<recent[i]->message<<endl;
				}
			}
			else
			{
...
//				cout<<datasize-1<<endl;
		...
		else if(opcode=='p')
		{
			...
			{
				cout<<i<<"|"<<(*(excerpt.begin()+i))->id<<"|"<<(*(excerpt.begin()+i))->timestamp<<"|"
				<<(*(excerpt.begin()+i))->category<<"|"<<(*(excerpt.begin()+i))->message<<endl;
			}
		}
